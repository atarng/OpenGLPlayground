...
vector<int> TextureLoader::LoadFiles(vector<string> files_to_load){
    ...
    for (int i = 0; i < files_to_load.size(); i++) {
        ...
        if (strncmp(filecode, "DDS ", 4) == 0) {
            ret_vector.push_back( LoadDDS(fp) );
        }
        else {
            printf("Unhandled Image Type.");
        }
...
//int TextureLoader::LoadDDS(string imagePath) {}
int TextureLoader::LoadDDS(FILE* fp) {
